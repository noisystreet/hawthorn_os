...
向量偏移 **`0x400`** 在 **AArch64 EL0** 执行 `svc` 时进入。当 **`ESR_EL1.EC == 0x15`**（AArch64 SVC）时，陷入处理进入 `syscall::dispatch`（寄存器约定与调用号见共享 crate **`hawthorn_syscall_abi`**；综述见 [KERNEL.md §3.8 — 系统调用接口](./KERNEL.md#38-系统调用接口syscall)）。
