...
本文档描述 **山楂（hawthorn）** 在 **首发板卡（香橙派 5 / RK3588）** 上的启动假设、构建入口与尚未收敛项；与 [架构.md](./架构.md)、[内核.md](./内核.md) 配合阅读。
...
当前 **开放决策**见 [架构.md §10](./架构.md)（启动与镜像格式、TF-A / U-Boot、FDT 等）。实现前需在 `bsp/orangepi5-rk3588/` 与 [引导.md](./引导.md) 中固定下列之一或组合：
...
正式布局确定后，在本节或 **《引导》**（`引导.md`）中更新并链接 `linker.ld`。
...
| 文档 | 内容 |
|------|------|
| [引导.md](./引导.md) | 启动信息块与引导阶段（骨架） |
| [系统调用ABI.md](./系统调用ABI.md) | 系统调用 ABI（骨架） |
| [平台.md](./平台.md) | 平台 tier 列表 |
| [术语.md](./术语.md) | 术语 |
| [接口.md](./接口.md) | 对外 API 索引（占位） |

---

## 6. 公开 API 说明

用户态与内核之间的 **稳定接口** 以 `syscall_abi`（未来 crate）与 [系统调用ABI.md](./系统调用ABI.md) 为准；crate 级 API 索引见 [接口.md](./接口.md)。
