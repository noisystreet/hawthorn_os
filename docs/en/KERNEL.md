...
**Current implementation (QEMU virt):** `trap` routes **EL1 and EL0** SVC (`ESR.EC == 0x15`) to `syscall::dispatch` (`x8` number, `x0–x5` args, `x0` retval; errors use the **negative errno** convention in that crate). Numbers include **`SYS_WRITE`**, **`SYS_READ`** (stub → `ENOSYS`), **`SYS_YIELD`**, **`SYS_GETPID`**, **`SYS_EXIT`**, **`SYS_SLEEP`**, endpoint MVP (create/destroy/call/recv/reply), and **`SYS_ABI_INFO`**: `hawthorn_syscall_abi::abi_info_word()` is defined as **low 32 bits = `ABI_VERSION`**, **high 32 bits = `ABI_CAP_*` mask** (not an errno encoding). **Canonical ABI and number-space rules:** [SYSCALL_ABI.md](./SYSCALL_ABI.md) (DRAFT-1.0).
