...
- M7-W2 (IPC MVP) 🔄: `endpoint` + `SYS_ENDPOINT_{CREATE,DESTROY,CALL,RECV,REPLY}`; **Phase 1** uses **≤32-bit** inline requests (`ENDPOINT_INLINE_REQ_MASK`), **full `u64` `reply`**, and **`call` / `recv` block** when the peer is not ready (rendezvous); `hawthorn_syscall_abi::user` has AArch64 stubs. True **≤128B** payloads and a **dual-EL0** roundtrip remain follow-up work.
