...
| **EL0 `trap_frame` pointer** | The stub passes `&mut TrapFrame` in **x2**, but **x2 is caller-saved**: after `schedule()` / `context_switch`, **x2 may no longer hold that pointer** when resuming syscall/IRQ handling. User tasks store **`el0_trap_frame`** in the TCB (set on `El0Sync`/`El0Irq` entry). Before syscall return and after EL0 IRQ (+ possible reschedule), **`saved_elr` / `saved_spsr` / `saved_sp_el0`** are copied from the TCB into the **in-memory trap frame** (+248/+256/+264) so the stub’s `ldr`/`msr` matches the TCB (`msr` alone is not enough—the stub reloads from memory). |
