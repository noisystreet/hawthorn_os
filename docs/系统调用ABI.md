...
| 返回值 | `x0` = 成功时为非负结果；**错误时为负 errno**（与 Linux aarch64 惯例一致，见 `hawthorn_syscall_abi::Errno::as_u64`）。说明见 [缺陷修复笔记.md](./缺陷修复笔记.md) 第 1 节。 |
...
冻结前仍可按 [待办.md](./待办.md) 与 issue 计划调整编号与参数。
