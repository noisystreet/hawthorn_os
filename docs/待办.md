...
本页列出计划在仓库内实现的 **新功能与运行时能力**；设计背景与约束见 [架构.md](./架构.md)、[内核.md](./内核.md)、[移植.md](./移植.md)。子项尽量 **可独立开 issue**；用 `- [ ]` / `- [x]` 在 PR 中跟踪。
...
- [ ] 在 `kernel/` 内划分 crate 内模块（建议名：`ids`、`caps`、`task`、`wait` 等），并在《内核》文档中画 **单向依赖**（禁止 `kernel` → `servers`）。
...
- [x] **EL 选择**：文档 + 代码一致（如长期 EL1 或先 EL2 再降级）；与 [引导.md](./引导.md) 交叉引用。
...
- [ ] **AP 入口**：`spin-table` 或 PSCI 路径择一，与 [移植.md](./移植.md) 真机假设对齐。
...
- [x] 在 [系统调用ABI.md](./系统调用ABI.md) 定义 **编号空间**、**DRAFT-x.y** 与 **STABLE-x** 命名规则（当前 **DRAFT-1.0**，`ABI_VERSION = 1`）。
- [x] **寄存器约定**：`SVC #0`，`x8` syscall 号、`x0–x5` 参数、`x0` 返回 / **负 errno**；与 AArch64 惯例差异见 [系统调用ABI.md](./系统调用ABI.md) §2。
- [x] **版本探测 syscall**：`SYS_ABI_INFO` / `abi_info_word()` 与 `ABI_CAP_*`（见 [系统调用ABI.md](./系统调用ABI.md) §3）。

### `syscall_abi` crate

- [x] 常量：`SYS_*` 编号、`SYSCALL_MAX_ARGS`、`SYSCALL_NR_CORE_MAX`、`SYSCALL_DISPATCH_TABLE_LEN`、`Errno` 与 `errno_from_ret`（`From<u64>` 全量枚举另需时再补）。
- [ ] **用户侧封装**（可选子 crate）：内联 asm 或 `compiler_builtins` 兼容的封装函数（与 `no_std` 用户态对齐）。

### 陷入与返回

- [x] **SVC 入口**：内核 `syscall::dispatch`；非法号 / 未实现槽位返回 `ENOSYS`（见实现）。
...
- [ ] **入口地址**：与 U-Boot / TF-A 移交假设一致（`ASSUME-*` 编号写进 `引导.md` / 对应文档）。
...
- [ ] `servers/` 布局：`init`、可选 `name_server`；**启动顺序**（谁拉谁）写进《内核》或《引导》对应章节。
...
- **当前里程碑的 PR / issue 顺序**（含 GitHub 链接）：[PR与议题计划.md](./PR与议题计划.md)。
