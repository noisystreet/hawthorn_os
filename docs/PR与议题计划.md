...
- M7-W2（IPC MVP）🔄：`endpoint` + `SYS_ENDPOINT_{CREATE,DESTROY,CALL,RECV,REPLY}`；**Phase 1** 为 **≤32 位** 内联请求（`ENDPOINT_INLINE_REQ_MASK`）、**完整 `u64` `reply`**，`call` / `recv` 在对手未就绪时 **阻塞**（rendezvous）；`hawthorn_syscall_abi::user` 提供 AArch64 存根。真正 **≤128B** 与 **双 EL0** 任务回环仍为后续。
